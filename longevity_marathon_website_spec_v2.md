...
**Phase 2 (built August 2026) — Supabase data platform.**

Implemented as follows (see `supabase/`, `scripts/`, and `.github/workflows/`):

- Supabase (managed Postgres + auto-generated REST API + Auth + Edge Functions) is the single backend for all non-editorial data. This is additive infrastructure, not a hosting migration — GitHub Pages continues to serve `site/`.
- **Garmin ingestion:** a scheduled GitHub Action (not an Edge Function as originally planned — the only well-maintained unofficial Garmin client after Garmin's 2026 auth changes is Python's `python-garminconnect`, and Actions provide the right runtime plus visible logs) pulls Garmin Connect data daily into private Postgres staging tables, per §13.2. Garmin's rotating OAuth token bundle lives in a private table and is rewritten by each run; raw records are never public and never committed to the site repo. Garmin's official Health API still requires partner approval, so the unofficial-client risk (breakage on Garmin auth changes) is accepted and documented.
- **App-logged data (built 2026-08-16):** the companion logging surface is a private, authenticated page at `/log/` (mobile-first static HTML, no SDK — plain fetch against Supabase REST + Auth). It captures the four daily moments from plan v3 §4.2 (morning, pre-run, post-run, next-morning), the weekly decision, and the issue register, writing directly to Supabase under row-level security. Access: Supabase Auth with public signups disabled and a single author user; every policy and private view additionally checks the author's email claim (`is_author()`), so even a stray authenticated session sees nothing. The anon key embedded in the page is public by design; it grants nothing without the author JWT.
- **Adaptive plan v3 schema (2026-08-16):** `plan_week` now carries plan v3 (16 weeks, effective 17 Aug 2026; the v2 placeholder seed was removed — its public snapshot page remains). New tables: `plan_session` (day-level sessions, replacing the hand-edited dashboard table), `checkin`, `issue`, `weekly_decision`, and `gate_rule` (adaptation thresholds as data). `garmin_daily` gained nightly HRV columns (verified available from Garmin via `get_hrv_data`). SQL views compute the green/amber/red gate proposals (`v_daily_status`, `v_today_status`) — red is only ever reported by the runner, never derived from wearables. Second recorded auto-publish exception (author's decision, 2026-08-16): daily gate *statuses* (the G/A/R letter only, no symptom detail) and nightly HRV aggregates publish automatically, same wellness-aggregate class as resting HR and sleep. Symptom detail, pain maps, GI logs, and clinician notes stay private; weekly decisions publish only when explicitly flagged `is_public` from `/log/`.
- **Curation:** the author selects what becomes public by flipping `review_state`/`is_public` in the private tables (Supabase Studio); export views expose only approved rows and privacy-safe fields, consistent with the ingest-privately/publish-selectively rule in §13.2 and §13.1. One recorded exception (author's decision, 2026-08-14): daily wellness aggregates (resting HR, sleep duration/score, steps, VO₂ max estimate) auto-approve on ingestion — they carry no location data and the project treats them as always-current trends. Extended 2026-08-16 (author's decision): activities also auto-approve, since `export_runs` publishes no route, coordinates, or exact start times (date and duration only) — the manual gate added curation latency without adding privacy. Manual override remains: flipping a row to `private` in Studio sticks across re-ingestion, and raw payloads with route data are never exported regardless.
- **Site build:** a GitHub Action queries the curated export views, rewrites marked regions of the affected pages plus `site/data/*.json`, commits, and GitHub Pages redeploys. The public site remains fully static; Supabase data is never queried client-side by site visitors.
- **Newsletter (§13.4):** implemented as a Supabase Edge Function (subscribe/confirm/unsubscribe with double opt-in, honeypot spam protection, consent timestamp and source) sending via Resend from `newsletter@marathonlongevity.run`. The subscriber list lives in the same Postgres under RLS; the sending key is send-only.
