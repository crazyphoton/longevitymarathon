...
- **Curation:** the author selects what becomes public by flipping `review_state`/`is_public` in the private tables (Supabase Studio); export views expose only approved rows and privacy-safe fields, consistent with the ingest-privately/publish-selectively rule in §13.2 and §13.1.
